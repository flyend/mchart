/**
 * Class Chart.Legend
 * new Chart.Legend(Canvas, data, options = {})
*/
"use strict";
...
;(function(global, Graph){
    ...
    Legend.prototype = {
        ...
        init: function(canvas, series, options){
            ...
            //Chart.scale(this.context, this.canvas.width, this.canvas.height, DEVICE_PIXEL_RATIO);
        ...
        translate: function(){
            var options = this.options,
               ...
               // margin = pack("number", options.margin, 0),
                ...
                chartWidth = pack("number", this.canvas.width / DEVICE_PIXEL_RATIO, width),
                chartHeight = pack("number", this.canvas.height / DEVICE_PIXEL_RATIO, this.height);
            ...
            //console.log(y, x, chartHeight, this)
            //y = Math.max(y, margin);
            //x = Math.max(x, margin);
            //var linePixel = fixLinePixel(x, y, x, y, 0);
        ...
        setLabels: function(){
            ...
            this.width = (sumWidth + padding * 2 - itemDistance) * (length !== 0);//fix width
            if(lineNumber < 4){
        ...
        formatter: function(){
            ...
            this.items.forEach(function(item){
                ...
                if(item.disabled){
                    ...
                    y = symbolBBox.y;// - bbox.height;// + symbolBBox.height / 2;
                    y += (symbolBBox.height - bbox.height) / 2;
        ...
        draw: function(){
            ...
            if(this.data.length){
                ...
                if(isClip){
                    //this.translateY = -(this.lineNumber - 3) * (items[0].height + options.padding);
                }
        ...
        onScroll: function(callback){
            ...
            var animateTo = function(clickedItem){
                ...
                if(defined(Graph.Animation)/* && (endY < legend.height)*/){
                    var value = 0;
                    Animation ? Animation.stop() : (Animation = new Graph.Animation());
                    [[{ }, function(timer){
            ...
            var onClick = function(e){
                ...
                for(var i = 0; i < buttons.length; i++){
                    var item = buttons[i];
                    if(item && Chart.Intersection.rect({
                        x: ex,
                        y: ey
                    }, {
                        x: item.p0.x - 10 + x,
        ...
        extend(Legend.prototype, {
            ...
            onState: function(callback){
                ...
                if(canvas.nodeType === 1){
                    canvas.removeEventListener("mousemove", onState, false);
                    canvas.addEventListener("mousemove", onState, false);
                }
                onState = null;
                return this;
            }
